...
const AuthController = {
    ...
    async login(req, res) {
        try {
            ...
            if (!valid) return res.status(401).json({ message: 'Email atau password yang kamu masukkan salah.' });
    ...
    async refresh(req, res) {
        try {
            ...
            if (!refresh_token) return res.status(422).json({ message: 'Refresh token harus ada.' });

            const stored = await TokenModel.findRefreshToken(refresh_token);
            if (!stored) return res.status(401).json({ message: 'Refresh token tidak sesuai atau sudah kadaluarsa.' });
            ...
            if (!user) return res.status(404).json({ message: 'User tidak ada/tidak ditemukan.' });
